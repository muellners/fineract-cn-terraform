resource "aws_alb_target_group" "identity_target" {
  ...
  health_check {
    ...
    matcher             = "400"
    timeout             = "3"
    path                = var.identity.health_check_path
...
resource "aws_alb_listener_rule" "identity_subdomain_rule" {
  listener_arn = aws_alb_listener.http-listener.arn

  action {
    target_group_arn = aws_alb_target_group.identity_target.arn
    type = "forward"
  }

  condition {
    host_header {
      values = [var.identity.service_url]
    }
  }
}

resource "aws_route53_record" "identity_domain_record" {
  name = var.identity.service_url
  type = "A"
...
resource "aws_cloudwatch_log_group" "identity_log_group" {
  name              = "/ecs/akounts/identity"
  retention_in_days = 30

  tags = {
    Name = "identity-log-group"
  }
}

data "template_file" "identity_tasks_data" {
  template = file("./templates/ecs/service.json.tpl")

  vars = {
    service_name = var.identity.service_name
    service_url = var.identity.service_url
    service_image = var.identity.image
    service_port = var.identity.port
    service_cpu = var.identity.cpu
    service_memory = var.identity.memory
    service_log_group = var.identity.log_group
...
resource "aws_ecs_task_definition" "identity_task" {
  family                   = "identity_task"
  cpu                      = var.identity.cpu
  memory                   = var.identity.memory
  container_definitions    = data.template_file.identity_tasks_data.rendered
}

resource "aws_ecs_service" "identity_service" {
  name            = "identity_service"
  cluster         = aws_ecs_cluster.akounts_cluster.id
  task_definition = aws_ecs_task_definition.identity_task.arn
  desired_count   = var.eureka.desired_count

  load_balancer {
    target_group_arn = aws_alb_target_group.identity_target.id
    container_name   = "identity"
    container_port   = var.identity.port
  }
